...
class Moderation(commands.Cog, name="moderation"):
    ...
    async def kick(
        self, context: discord.Interaction, user: discord.User, *, reason: str = "Not specified"
    ) -> None:
        """
        ...
        if member.guild_permissions.administrator:
            ...
            try:
                embed = discord.Embed(
                    description=f"**{member}** was kicked by **{context.user}**!",
                    color=0xBEBEFE,
                )
    ...
    async def nick(
        self, context: discord.Interaction, user: discord.User, *, nickname: str = None, hamster: str = None
    ) -> None:
        """
        ...
        :param hamster: Name a hamster type
        """
        member = context.guild.get_member(user.id) or await context.guild.fetch_member(
            user.id
        )
    ...
    async def ban(
        ...
        try:
            if member.guild_permissions.administrator:
                ...
                embed = discord.Embed(
                    description=f"**{member}** was banned by **{context.user}**!",
                    color=0xBEBEFE,
                )
                embed.add_field(name="Reason:", value=reason)
                await context.response.send_message(embed=embed)
                try:
                    await member.send(
                        f"You were banned by **{context.user}** from **{context.guild.name}**!\nReason: {reason}"
                    )
                except:
    ...
    # async def warning(self, context: discord.Interaction) -> None:
    #     """
    ...
    #         user.id, context.guild.id, context.user.id, reason
    #     )
    #     embed = discord.Embed(
    #         description=f"**{member}** was warned by **{context.user}**!\nTotal warns for this user: {total}",
    #         color=0xBEBEFE,
    #     )
    #     embed.add_field(name="Reason:", value=reason)
    #     await context.response.send_message(embed=embed)
    #     try:
    #         await member.send(
    #             f"You were warned by **{context.user}** in **{context.guild.name}**!\nReason: {reason}"
    #         )
    #     except:
    #         # Couldn't send a message in the private messages of the user
    #         await context.response.send_message(
    #             f"{member.mention}, you were warned by **{context.user}**!\nReason: {reason}"
    #         )
    ...
    #     self, context: discord.Interaction, user: discord.User, warn_id: int
    # ) -> None:
    #     """
    #     Warns a user in his private messages.
    ...
    # async def warning_list(self, context: discord.Interaction, user: discord.User) -> None:
    #     """
    ...
    async def purge(self, context: discord.Interaction, amount: int) -> None:
        """
    ...
    async def hackban(
        self, context: discord.Interaction, user_id: str, *, reason: str = "Not specified"
    ) -> None:
        """
        ...
        try:
            ...
            embed = discord.Embed(
                description=f"**{user}** (ID: {user_id}) was banned by **{context.user}**!",
                color=0xBEBEFE,
            )
            embed.add_field(name="Reason:", value=reason)
            await context.response.send_message(embed=embed)
    ...
    async def archive(self, context: discord.Interaction, limit: int = 10) -> None:
        """
        ...
        with open(log_file, "w", encoding="UTF-8") as f:
            ...
            async for message in context.channel.history(
                ...
                f.write(
                    f"{message.created_at.strftime('%d.%m.%Y %H:%M:%S')} {message.user} {message.id}: {message.clean_content} {attachments_text}\n"
                )
        f = discord.File(log_file)
        await context.response.send_message(file=f)
        os.remove(log_file)
